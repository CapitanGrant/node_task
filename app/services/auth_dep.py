...
# получаем Keycloak из app.state
...
async def get_current_user(token: str = Depends(get_token_from_cookie), keycloak: KeycloakClient = Depends(get_keycloak_client)) -> dict | None:
    if not token:
